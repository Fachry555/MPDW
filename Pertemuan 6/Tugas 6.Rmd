...
```
```{r}
data <- read_excel("C:/Users/LENOVO/Downloads/Fachry (1).xlsx")
data
```
...
```
Grafik density plot menunjukkan sebaran harga saham yang bersifat bimodal, dengan dua puncak utama di kisaran Rp 10.500–11.000 dan sekitar Rp 12.000. Hal ini mengindikasikan bahwa selama periode pengamatan, harga saham PT Indofood cenderung sering berada di dua rentang tersebut. Sebaran yang cukup lebar menggambarkan adanya fluktuasi harga yang relatif tinggi, sementara area dengan kepadatan rendah di antara kedua puncak menandakan adanya pergeseran level harga pada waktu tertentu.
...
```
Plot ACF menunjukkan bahwa data harga saham memiliki autokorelasi yang tinggi pada beberapa lag awal, dengan pola penurunan yang lambat. Hal ini menandakan bahwa nilai harga pada waktu tertentu sangat dipengaruhi oleh harga pada periode sebelumnya (adanya efek memori). Pola seperti ini mengindikasikan bahwa data belum bersifat stasioner dan kemungkinan memerlukan proses differencing sebelum dilakukan pemodelan ARIMA
...
```
Karena dalam selang kepercayaan -1 < lamda < 1.88 terdapat lamda = 1, artinya model kita stasioner dalam ragam.
...
```
model dengan ma(1) tidak signifikan.
...
```
model dengan AR(1) tidak signifikan
...
aic.arima<-data.frame(
  ...
  "AIC"=c(model1.da$aic, model2.da$aic, model3.da$aic))

aic.arima
```
Model ARIMA(0,1,1) memiliki nilai AIC ter-rendah.
...
```
Asumsi Non-Autokorelasi terpenuhi, hal ini dapat dilihat dari nilai p-value>0.05.
...
```
P-value<0.05, artinya kita menolak h0. hal ini menunjukkan pelanggaran asumsi normalitas residual.
...
```
Asumsi nilai harapan residual=0 terpenuhi.

# Overfitting Model

Overfitting model dilakukan pada model :
1. ARIMA(2,1,1)
2. ARIMA(1,1,2)
...
mape_train
```
Nilai MAPE pada train kurang baik
